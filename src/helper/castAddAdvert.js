export function castAddAdvert(formData, user, portfolio_id) {
  ...
  if (formData?.product?.length > 0) {
    if (formData?.addProduct === "YesAddProduct") {
      // Loop through each product and set productTitle
      formData.product.forEach((product, index) => {
        formData.product[index].productTitle = formData.productTitle;
      });

      console.log(formData);
      // Append the modified product array to the form data
